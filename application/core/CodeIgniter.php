...
class MY_Config Extends CI_Config {
    ...
    var $routes_path = ''; // Set in the constructor below
    ...
    function __construct() {
        ...
        $this->routes_path = APPPATH . 'config/routes.php';
    }
...
function routes_config_update($config_array = array()) {
    if (!is_array($config_array) && count($config_array) == 0) {
        return FALSE;
    }

    @chmod($this->routes_path, FILE_WRITE_MODE);

    // Is the config file writable?
    if (!is_really_writable($this->routes_path)) {
        show_error($this->routes_path . ' does not appear to have the proper file permissions.  Please make the file writeable.');
    }

    // Read the config file as PHP
    require $this->routes_path;
    ...
    if (!$fp = fopen($this->routes_path, FOPEN_WRITE_CREATE_DESTRUCTIVE)) {
        return FALSE;
    }
    ...
    @chmod($this->routes_path, FILE_READ_MODE);
