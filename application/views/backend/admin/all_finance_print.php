...
function generateRow(){
		$contents = '';
	 	$day = date('Y-m-d');
	$sql= " SELECT * FROM `treatment`  where paid='No' and cdate='$day'";
...
// P =portriat   L=landscape
    ...
    $pdf->SetTitle('All Unpaid finance information');  
    ...
    $content .= '
      	...
      	<h2 align="center"> Toady Service charges Collection on '.$new.' </h2>
    ...
    $content .=generateRow() ; 
    		...
    		$result1 = mysql_query("SELECT sum(card) FROM treatment where paid='No' and cdate='$day'") or die(mysql_error());
    		while ($row1 = mysql_fetch_array($result1)) {
				$card2 = $row1['sum(card)'];
				} 
           $result2 = mysql_query("SELECT sum(lab) FROM treatment where paid='No' and cdate='$day'") or die(mysql_error());
    		while ($row2 = mysql_fetch_array($result2)) {
			$lab2  =	$row2['sum(lab)'];
			} 
					
	      $result3 = mysql_query("SELECT sum(Imaging) FROM treatment where paid='No' and cdate='$day'") or die(mysql_error());
    		while ($row3 = mysql_fetch_array($result3)) {
				$Imaging2 = $row3['sum(Imaging)'];
				}
		 $result4 = mysql_query("SELECT sum(surgery) FROM treatment where paid='No' and cdate='$day'") or die(mysql_error());
    		while ($row4 = mysql_fetch_array($result4)) {
				$surgery2 = $row4['sum(surgery)'];
				}
		$result5 = mysql_query("SELECT sum(drugs) FROM treatment where paid='No' and cdate='$day'") or die(mysql_error());
    		while ($row5 = mysql_fetch_array($result5)) {
				$drugs2 = $row5['sum(drugs)'];
				}
		$result6 = mysql_query("SELECT sum(Bed_fee) FROM treatment where paid='No' and cdate='$day'") or die(mysql_error());
    		while ($row6 = mysql_fetch_array($result6)) {
				$Bed_fee2  = $row6['sum(Bed_fee)'];
				}
										 
      $result7 = mysql_query("SELECT sum(other) FROM treatment where paid='No' and cdate='$day'") or die(mysql_error());
    ...
    $pdf->Output('All Unpaid finance information.pdf', 'I');
