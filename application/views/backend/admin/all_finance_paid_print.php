...
function generateRow($paid){
		$contents = '';
	 	
	$sql="SELECT * FROM `treatment`  where paid='$paid'";
	 //$sql="select * from treatment  where paid='Yes'";
                   $result=mysql_query($sql) or die(mysql_error());
                        if(mysql_num_rows($result)<1)
                           {
	             
						 echo'<div style="color:red;font-size:17px;" ><img src="uploads/invalid.PNG" width="17px" height="17px"/>&nbsp; Yes unpaid service charge found!</div>';
	                          exit;
                              }
							  	   $i=1;
...
// P =portriat   L=landscape
    ...
    $pdf->SetTitle('All Paid finance Information');  
    ...
    $content .= '
      	...
      	<h2 align="center">  Fully or Partailly Made Service charge payment </h2>
    ...
    $content .=generateRow($paid) ; 
      ...
      $result7 = mysql_query("SELECT sum(other) FROM treatment where paid='Yes'") or die(mysql_error());
		...
		<h3 align="right;">  <b>Covered  Service Charge : Birr <u> '.number_format($paid_amount, 2).'</u> </b></h3>
    	<h3 align="right;">  <b>Uncovered Service charge :  Birr <u> '.number_format($unpaid_amount,2).'</u>   </b></h3> 
    ...
    $pdf->Output('All Paid finance Information.pdf', 'I');
