...
function generateRow($user ,$date){
		$contents = '';
		       ...
		       $sql="select * from treatment where  user ='$user' and  paid='No' and cdate='$date'";  
...
//////////////////////////////////////////////////////////////////////////////////////////////
    ...
    $content .= '
		...
		<b>This report is generated on '.$new.'  by  <u>'.$name.'</u> to summarize or declare that daily Service charge report</h2>
	<br><br>
    ...
    $content .=generateRow($user ,$date) ; 
	

		   $result1 = mysql_query("select sum(card) from treatment where  user ='$user' and  paid='No' and cdate='$date'") or die(mysql_error()); 		   
	
    		while ($row1 = mysql_fetch_array($result1)) {
				$card2 = $row1['sum(card)'];
				} 

		   $result2  =mysql_query("select sum(lab) from treatment where  user ='$user' and  paid='No' and cdate='$date'") or die(mysql_error());   		   

    		while ($row2 = mysql_fetch_array($result2)) {
			$lab2  =	$row2['sum(lab)'];
			} 
		   $result3 = mysql_query("select sum(Imaging) from treatment where  user ='$user' and  paid='No' and cdate='$date'") or die(mysql_error());   		   
	 
    		while ($row3 = mysql_fetch_array($result3)) {
				$Imaging2 = $row3['sum(Imaging)'];
				}
		   $result4 = mysql_query("select sum(surgery) from treatment where  user ='$user' and  paid='No' and cdate='$date'") or die(mysql_error());   		   
	 
    		while ($row4 = mysql_fetch_array($result4)) {
				$surgery2 = $row4['sum(surgery)'];
				}
		   $result5 = mysql_query("select sum(drugs) from treatment where  user ='$user' and  paid='No' and cdate='$date'") or die(mysql_error());   		   
	  
    		while ($row5 = mysql_fetch_array($result5)) {
				$drugs2 = $row5['sum(drugs)'];
				}

		   $result6  = mysql_query("select sum(Bed_fee) from treatment where  user ='$user' and  paid='No' and cdate='$date'") or die(mysql_error());   		   
	   
    		while ($row6 = mysql_fetch_array($result6)) {
				$Bed_fee2  = $row6['sum(Bed_fee)'];
				}
		   $result7  = mysql_query("select  sum(other) from treatment where  user ='$user' and  paid='No' and cdate='$date'") or die(mysql_error());  		   
