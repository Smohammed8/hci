...
$current_user = $this->session->userdata('admin_id');
						  ...
						  $s = "SELECT name,date_of_join FROM `admin` where admin_id='$current_user'";
                          $ress = mysql_query($s) or die(mysql_error());
                           while($rows = mysql_fetch_array($ress)){
                           $name = $rows['name']; 
						  // $date = $rows['date_of_join']; 
			                }
							
							
function generateRow($user, $date){

		$contents = '';
	 	
	$sql="SELECT * FROM `treatment`  where paid='No' and user='$user' and cdate='$date'";
                   $result=mysql_query($sql) or die(mysql_error());
                        if(mysql_num_rows($result)<1)
                           {
	             
						 echo'<div style="color:red;font-size:17px;" >
						 <img src="uploads/invalid.PNG" width="17px" height="17px"/>&nbsp; 
						No encounter were created today!</div>';
						   exit;
...
// P =portriat   L=landscape
    ...
    $pdf->SetTitle('Per user daily encounter report');  
 ...
 <img src="uploads/pdfheader.jpg" style="align:left;"> 
      	...
      	<h2 align="center">Per user daily encounter report - '.$new.'</h2>
			
		<p>  4=Oromia Region,  7=SNNP Region,8=Gambela Region <br>
	            19=Bunobedele zone, 10=Jimma zone,  09 = Elubabora zone   </p> 
				
				<b>This report is generated on '.$new.'  by  <b> <u>'.$name.'</u> </b>
		
		
		to provide hard copy backup in pdf as per user encounter report from HCI Application- Jimma University Medical Center</h2>
    ...
    $content .=generateRow($current_user,$new); 
	

    		$result1 = mysql_query("SELECT sum(card) FROM treatment where paid='No' and user='$current_user'") or die(mysql_error());
    		while ($row1 = mysql_fetch_array($result1)) {
				$card2 = $row1['sum(card)'];
				} 
           $result2 = mysql_query("SELECT sum(lab) FROM treatment where paid='No' and user='$current_user'") or die(mysql_error());
    		while ($row2 = mysql_fetch_array($result2)) {
			$lab2  =	$row2['sum(lab)'];
			} 
					
	      $result3 = mysql_query("SELECT sum(Imaging) FROM treatment where paid='No' and user='$current_user'") or die(mysql_error());
    		while ($row3 = mysql_fetch_array($result3)) {
				$Imaging2 = $row3['sum(Imaging)'];
				}
		 $result4 = mysql_query("SELECT sum(surgery) FROM treatment where paid='No' and user='$current_user'") or die(mysql_error());
    		while ($row4 = mysql_fetch_array($result4)) {
				$surgery2 = $row4['sum(surgery)'];
				}
		$result5 = mysql_query("SELECT sum(drugs) FROM treatment where paid='No' and user='$current_user'") or die(mysql_error());
    		while ($row5 = mysql_fetch_array($result5)) {
				$drugs2 = $row5['sum(drugs)'];
				}
		$result6 = mysql_query("SELECT sum(Bed_fee) FROM treatment where paid='No' and user='$current_user'") or die(mysql_error());
    		while ($row6 = mysql_fetch_array($result6)) {
				$Bed_fee2  = $row6['sum(Bed_fee)'];
				}
										 
      $result7 = mysql_query("SELECT sum(other) FROM treatment where paid='No' and user='$current_user'") or die(mysql_error());
    ...
    $pdf->Output('Per user daily encounter report.pdf', 'I');
