...
function generateRow($user,$date){
	 ...
	 $result = mysql_query("SELECT * FROM patients where user ='$user' and cdate ='$date'") or die(mysql_error());
...
// P =portriat   L=landscape
    ...
    $pdf->SetTitle('Per user daily report.pdf');  
    ...
    $content .= '
      	...
      	<h2 align="center"> Per user daily patient Registration report -'.$new.'</h2>
		...
		to provide hard copy backup in pdf as per user daily report from HCI Application- Jimma University Medical Center</h2>
    ...
    $pdf->Output('Per user daily report.pdf', 'I');
