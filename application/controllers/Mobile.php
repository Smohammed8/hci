...
class Mobile extends CI_Controller
{
    ...
    function get_students_of_class() {
        ...
        $students       =   $this->db->get_where('patients' , array('class_id' => $class_id))->result_array();
        ...
        foreach ($students as $row) {
            ...
            $data['image_url']  =   $this->crud_model->get_image_url('patients' , $row['student_id'] );
    ...
    function get_student_profile_information() {
        ...
        $student_profile=   $this->db->get_where('patients' , array('mrn_id' => $student_id))->result_array();

        foreach ($student_profile as $row) {
            $data['student_id'] =   $row['mrn_id'];
            $data['name']       =   $row['name'];
            $data['birthday']   =   $row['birthday'];
            $data['gender']     =   $row['sex'];
            $data['address']    =   $row['address'];
    ...
    function get_total_summary() {
        ...
        $response       =   array();
        $response['total_student']      = $this->db->count_all('patients');
    ...
    function get_children_of_parent() {
        ...
        $response       =   array();
        $parent_id      =   $this->input->post('parent_id');
        $response['children']       =   $this->db->get_where('patients' , array('mr_id' => $parent_id))->result_array();
        echo json_encode($response);
    }
    ...
    function login() {
        ...
        $query = $this->db->get_where('patients', array('email' => $email , 'password' => $password));
        if ($query->num_rows() > 0) {
            $row = $query->row();
            ...
            // update the new authentication key into user table
            $this->db->where('mrn_id' , $row->student_id);
            $this->db->update('patients' , array('authentication_key' => $authentication_key));
            echo json_encode($response);
            return;
    ...
    function reset_password() {
        ...
        if ($query->num_rows() > 0) 
        {
            ...
            $this->db->where('email' , $email);
            $this->db->update('patients' , array('password' => $new_password));
            $response['status']         = 'true';
        }
